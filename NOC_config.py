...
def rsdp_pw_create(bsr01, bsr02):
    ...
    if s1 != False and s2 != False:
        ...
        if slct == 'y':
            pgsql.write(pg)
            print(f'{bc.GREEN}[!]{bc.ENDC} == Данные записаны в БД ==')
            cfg1 = ja2.cfg_render("radio_sdp_pw_create/bsr01.cfg", sdp=sdp, a_bsa=a_bsa, a_bsa_ipb=a_bsa_ipb, bin_port1=bin_port1)
            cfg11 = ja2.cfg_render("radio_sdp_pw_create/bsr01-pw.cfg", sdp=sdp, ring=ring, a_bsa=a_bsa)
            result01 = ssh.invoke(cfg1, s1)
            result1 = ssh.invoke(cfg11, s1)
            print(f'{bc.GREEN}[!]{bc.ENDC} == На BSR01 настроено ==')
            cfg2 = ja2.cfg_render("radio_sdp_pw_create/bsr02.cfg", sdp=sdp, b_bsa=b_bsa, b_bsa_ipb=b_bsa_ipb, bin_port2=bin_port2)
            cfg22 = ja2.cfg_render("radio_sdp_pw_create/bsr02-pw.cfg", sdp=sdp, ring=ring, b_bsa=b_bsa)
            result02 = ssh.invoke(cfg2, s2)
            result2 = ssh.invoke(cfg22, s2)
            print(f'{bc.GREEN}[!]{bc.ENDC} == На BSR02 настроено ==')
            log.write(f'==Данные записаны в БД!==\n{pg}\n{result01}\n{result1}\n{result02}\n{result2}', 1)
            if a_bsa == b_bsa:
                cfg3 = ja2.cfg_render("radio_sdp_pw_create/bsa-act-act.cfg", ring=ring)
                result_aa = telnet.huawei(cfg3, a_bsa_ip, radctl.username, radctl.password)
                print(f'{bc.GREEN}[!]{bc.ENDC} == На BSA-{a_bsa} настроено ==')
                log.write(result_aa, 1)
            else:
                cfg4 = ja2.cfg_render("radio_sdp_pw_create/bsa-act.cfg", b_bsa=b_bsa, b_bsa_ipb=b_bsa_ipb, ring=ring)
                result_a = telnet.huawei(cfg4, a_bsa_ip, radctl.username, radctl.password)
                print(f'{bc.GREEN}[!]{bc.ENDC} == На BSA-{a_bsa} настроено ==')
                cfg5 = ja2.cfg_render("radio_sdp_pw_create/bsa-bkp.cfg", a_bsa=a_bsa, a_bsa_ipb=a_bsa_ipb, ring=ring)
                result_b = telnet.huawei(cfg5, b_bsa_ip, radctl.username, radctl.password)
